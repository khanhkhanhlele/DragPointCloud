...
PATH_TO_MITSUBA2 = "/home/anguyen/.conda/envs/DPC/bin/mitsuba"  # mitsuba exectuable
...
def exr_to_png(exr_file, png_file):
    ...
    # Lấy header của file EXR để biết kích thước hình ảnh
    header = exr_file.header()
    dw = header['dataWindow']
    width = dw.max.x - dw.min.x + 1
    height = dw.max.y - dw.min.y + 1

    # Định nghĩa kiểu dữ liệu của các kênh màu
    FLOAT = Imath.PixelType(Imath.PixelType.FLOAT)

    # Đọc dữ liệu cho các kênh R, G, B
    red_str = exr_file.channel('R', FLOAT)
    green_str = exr_file.channel('G', FLOAT)
    blue_str = exr_file.channel('B', FLOAT)

    # Chuyển dữ liệu thành các mảng numpy
    ...
    # Kết hợp các kênh R, G, B thành một hình ảnh duy nhất
    img = np.stack([red, green, blue], axis=-1)

    # Chuẩn hóa giá trị màu về khoảng [0, 255]
    img = np.clip(img, 0, 1)
    img = (img * 255).astype(np.uint8)

    # Tạo ảnh PNG và lưu lại
