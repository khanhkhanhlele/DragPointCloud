...
class LION(object):
    ...
    def sample(self, num_samples=10, clip_feat=None, save_img=False):
        ...
        anchor_points = [torch.tensor([-0.9, 2.1, 0.3]).to('cuda')]
        target_points = [torch.tensor([-1.5, 2.1, 0.3]).to('cuda')]
                ...
                with torch.autocast(device_type='cuda'):
                    ...
                    if i >= 44:
                        ...
                        for j in range(len(anchor_points)):
                            ...
                            loss = loss + F.mse_loss(v[mask], d[mask]) + 20. * F.mse_loss(v[~mask], v_gt[~mask])
                    all.append(x_noisy)
