        ...
        for i, t in enumerate(timesteps):
            with torch.autocast(device_type='cuda'):
                ...
                if i == 0: 
                    ...
                    x_noisy = self.scheduler.step(noise_pred, t, x_noisy).prev_sample
                if i >= 44:
                    loss = torch.tensor(0.0, device=x_noisy.device, requires_grad=True)
                    ...
                    # x_noisy = x_noisy.reshape((-1, 4))
                    # points = x_noisy[:, :3]
                    # # while not check_anchor_reach_target(anchor_points, target_points, 0.2):
                    # points, anchor_points = point_tracking(anchor_points, points, target_points, 1.8)
                    # x_noisy[:, :3] = points
                    # x_noisy = x_noisy.reshape(initial_shape)
                    ##
                    
                    points = x_noisy.reshape((-1, 4))[:, :3]
                    _, mask_list = get_handle_points(anchor_points, points, 1.8)
                    for j in range(len(anchor_points)):
                        anchor, target = anchor_points[j], target_points[j]
                        mask = mask_list[j]
                        d = (target - anchor) #/ torch.norm(target - anchor)
                        print(torch.norm(target - anchor))
                        #reshape d as v
                        d = d.repeat(num_points, 1)
                        loss = loss + F.mse_loss(v[mask], d[mask])
                        print(loss)
                    print("END")
