...
class LION(object):
    ...
    #prepare condition
    @torch.no_grad()
    def prepare_condition(self, num_samples, latent_shape, timesteps, clip_feat=None, device='cuda'):
    ...
    # @torch.no_grad()
    def sample(self, num_samples=10, clip_feat=None, save_img=False, index=0, device='cuda'):
        ...
        condition_input = self.prepare_condition(num_samples, latent_shape, timesteps, clip_feat,device=device)
        ...
        # anchor_points = [torch.tensor([-0.9, 2.1, 0.3]).to(device)]
        # target_points = [torch.tensor([-1.5, 2.1, 0.3]).to(device)]
        ...
        with torch.no_grad():
            ...
            # print(f"User anchor points: {anchor_points}")
                ...
                with torch.autocast(device_type=device):
                    ...
                    if i >= 44:
                        for epoch in range(1):
                            ...
                            v_pred = x_noisy_prev - x_noisy #1, 8192
                            ...
                            for j in range(len(anchor_points)):
                                ...
                                loss6 = F.mse_loss(torch.norm(v[mask],dim=1), torch.norm(d[mask], dim=1))
                                # print(f"loss1: {loss} \t Loss2: {loss2}")
                                loss = loss + loss2 + 1. * loss6

                            # Tính backward cho toàn bộ các timestep và epoch
                            print(loss)
        ...
        torch.save(all, 'all_tensor_list_v2.pt')
        print("Saved sampled_list to all_tensor_list_v2.pt")
        torch.cuda.empty_cache()
        ...
        with torch.no_grad():
            ...
            # sampled_list.append(x_edit)
            # output_dict['z_local'] = x_edit
